...
public class TPSSchedulerConcurrentTest extends SchedulerTestBase {
    ...
    public void test1x1() throws Exception
    {
        ...
        // run it for 2 messages again, just for fun.
    ...
    public void testManyP_oneC() throws Exception {
        ...
        int nProducers = 5;
        int totalMsgs = 0; // there could be dupes in allExpectedMessages
        ...
        assertEquals("there were some leftover events that were unmatched: "+getStringFromEventArr(allExpectedMessages.toArray(new String[0])),
                0, allExpectedMessages.size());
    }
    ...
    public void testMany_Many() throws Exception {
        ...
        // now also verify that the sum of all numbers sent is equal to sum of all received
        int expectedSum = 0;
        for(int i=1;i<=nProducers;i++) {
    ...
    protected String getConsumerClass() { return CONSUMER_CLASS; }
    /** Subclasses should override if they want to substitute differnet kinds of producer */
    protected String getProducerClass() { return PRODUCER_CLASS; }


/** todo:
 * 1. Is producer setup correctly? it's not using the producer threadpool if we create a new thread internally every time
 * 2. verify that many-one messages are coming through appropriately (and not too orderly)
 * [producer-0]  sent event: [Value: 19, Producer: Thread-16, Consumer: null]
Adding message [Value: 19, Producer: Thread-16, Consumer: pool-7-thread-1]
[producer-2]  sent event: [Value: 40, Producer: Thread-17, Consumer: null]
Adding message [Value: 40, Producer: Thread-17, Consumer: pool-7-thread-1]
[producer-0]  sent event: [Value: 77, Producer: Thread-16, Consumer: null]
Adding message [Value: 77, Producer: Thread-16, Consumer: pool-7-thread-1]
[producer-1]  sent event: [Value: 16, Producer: Thread-15, Consumer: null]
Adding message [Value: 16, Producer: Thread-15, Consumer: pool-7-thread-1]
[producer-4]  sent event: [Value: 13, Producer: Thread-19, Consumer: null]
Adding message [Value: 13, Producer: Thread-19, Consumer: pool-7-thread-1]
[producer-0]  sent event: [Value: 8, Producer: Thread-16, Consumer: null]
Adding message [Value: 8, Producer: Thread-16, Consumer: pool-7-thread-1]
[producer-3]  sent event: [Value: 63, Producer: Thread-18, Consumer: null]
Adding message [Value: 63, Producer: Thread-18, Consumer: pool-7-thread-1]
[producer-0]  sent event: [Value: 26, Producer: Thread-16, Consumer: null]
Adding message [Value: 26, Producer: Thread-16, Consumer: pool-7-thread-1]
[producer-3]  sent event: [Value: 92, Producer: Thread-18, Consumer: null]
